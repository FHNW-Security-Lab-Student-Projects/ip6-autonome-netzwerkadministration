...
Before defining the term "AI Agent" we must understand what an agent is.
From a very basic perspective, an agent is an entity which is able to produce some effect or make changes in its
environment \parencite{CASTELFRANCHI1998157}. With the rise of generative AI a new term "AI Agent" has emerged.
AI Agents can be defined as software entities which are designed to fulfill goal-oriented tasks \parencite{Sapkota_2026}.
This means we are are to define our goal (what we want to do / achieve) in natural language and 
...
The following Image illustrates the procedure of function calling using the OpenAI API.
...
As illustrated in Figure~\ref{fig:mcp_architecture-overview}, MCP functions similarly to a
