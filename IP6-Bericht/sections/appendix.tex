...
advertised OpenRouter price at run time; the rates themselves are not stored in the log. Because
all runs of one model on one day share the same rate pair, the rates could be recovered exactly
from the logged costs and token counts (\texttt{derive\_model\_prices.py}); every logged cost is
reproduced by its period's rate pair. \Cref{tab:recovered-rates} lists the rates in effect.
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width=\linewidth]{graphics/scenario_time_to_diagnosis.pdf}
\end{center}
