...
domain-scoped sub-tasks to the specialised sub-agents, each of which is backed by its own tool layer.
...
Models are grouped into \emph{tiers} according to their \emph{price} (for example a low, a mid and a high tier). The system is
...
It may correlate topology, live device state, historical snapshots and further
syslog data to localise the probable cause. It is also designed so 
investigations cannot be opened recursively. The result is a self-contained initial report produced from a syslog event without any human
involvement.
