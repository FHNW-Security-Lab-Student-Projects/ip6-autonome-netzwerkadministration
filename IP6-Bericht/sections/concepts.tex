...
messages and starts a self-contained initial investigation without any human trigger. A related question is how such
...
\begin{itemize}
    \item \textbf{Orchestrator (user-facing).} Coordinates an interactive troubleshooting session, splits
          the user's goal into sub-tasks, delegates them to the sub-agents and synthesises their findings into a final answer. Since the sub-agents are "stateless" the orchestrator
...
LLMs can be compared to each other using a variety of metrics. For this work we decided to use the \emph{price per million tokens} as a comparison metric (because cost is a primary research question of this work).
...
orchestrator then runs the same delegation pattern as the user-facing orchestrator with the exception of not having access to the configuration agent.
