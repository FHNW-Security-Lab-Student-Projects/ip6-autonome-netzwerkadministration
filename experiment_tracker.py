...
Pricing:
    Cost estimates use model_pricing.json if present (generated by sync_pricing.py),
    falling back to the hardcoded _FALLBACK_PRICING dict below.
    Run `uv run python sync_pricing.py` before an experiment batch to get current prices.
"""
...
# Fallback prices (USD per 1M tokens) used when model_pricing.json is absent.
# Run sync_pricing.py to generate an up-to-date file covering all OpenRouter models.
...
def _load_pricing() -> dict[str, tuple[float, float]]:
    """Load pricing from model_pricing.json; fall back to _FALLBACK_PRICING."""
    if _PRICING_FILE.exists():
        try:
            data = json.loads(_PRICING_FILE.read_text())
            return {
                model_id: (float(prices[0]), float(prices[1]))
...
# Loaded once at import time. Reflects model_pricing.json if present.
